...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-alpha-sb-install.ps1
#
# ---------
# Uninstall:
# ---------
...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-alpha-sb-install.ps1 -uninstall
...
cat 1>&2 <<'EOF'
# Usage:
#   ./monarch-alpha-sb-install.sh [ -E moudi:nthallen/keutsch-moudi.git ] [-n] [-h] [-S]
#
# -E <basename>:<URL>
...
exp_option='alpha-sb:nthallen/ALPHA-SB.git'
exp_base=''
exp_url=''
testmode=no
installagent=no
...
if [ $machine = Cygwin ]; then
  ...
  if [ -e "$cyglnk" ]; then
    ...
    [ "$cygperm" = "775" ] || {
      chmod 0775 "$cyglnk"
      echo "monarch-alpha-sb-install.sh: Permission fix applied for Cygwin Start Menu Link"
    }
  fi
  # Apply vi annoyance fix if it isn't already present
  [ -f ~/.exrc ] || {
    touch ~/.exrc
    echo "monarch-alpha-sb-install.sh: Installed vi startup error mitigation"
  }
  # Verify that the monarch group flight exists and user is a member
  id | grep -q '(flight)' || {
    if id | grep -q "(`hostname`+flight)"; then
      echo "monarch-alpha-sb-install.sh: The flight group 'flight' has been created,"
      ...
      echo "monarch-alpha-sb-install.sh: user '$myuser' does not appear to be a member"
      echo "of group 'flight'. If you have not run cygwin-monarch-alpha-sb-install.ps1,"
      echo "do so now. Otherwise try restarting the system and then rerun"
      echo "cygwin-monarch-alpha-sb-install.ps1."
    fi
    echo
  ...
  if grep -q "^flight:" /etc/passwd; then
    ...
    echo "monarch-alpha-sb-install.sh: Need to create user flight and group flight"
    $sudo addgroup flight
    echo "monarch-alpha-sb-install.sh: flight group created"
    $sudo adduser --disabled-password --gecos "flight user" --no-create-home --ingroup flight flight
    echo "monarch-alpha-sb-install.sh: flight user created"
  fi
...
umask 02
echo "monarch-alpha-sb-install.sh: checking permissions on /usr/local/src"
...
if [ $machine = Linux ]; then
  ...
  if [ $distro = Ubuntu ]; then
    echo "monarch-alpha-sb-install.sh: Checking prerequisites"
    sudo apt install cmake doxygen graphviz gdb gcc g++ git bison flex libncurses-dev openssh-server screen
  else
    echo "monarch-alpha-sb-install.sh: Not specifically configured for release '$release'."
...
'@
$setup_script | Out-File -FilePath "./monarch-alpha-sb-install.sh" -NoNewLine -Encoding ASCII

Write-Output "`nStarting standard install script: /usr/local/src/monarch-alpha-sb-install.sh`n"

# -Wait won't work here, because mintty.exe doesn't really exit until ssh-agent does
start-process C:\cygwin64\bin\mintty.exe -argumentlist "-h always /bin/bash --login /usr/local/src/monarch-alpha-sb-install.sh -S $exp_option"
