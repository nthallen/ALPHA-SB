...
void ALPHA_SB::printdR3(const dReal *d) {
  printdRN(d,3);
  // fprintf(ofp, ",%7.3lf,%7.3lf,%7.3lf", (double)d[0], (double)d[1], (double)d[2]);
}

void ALPHA_SB::printdRN(const dReal *d, int N) {
  int i;
  for (i = 0; i < N; ++i) {
...
void ALPHA_SB::dBodyAddDrag(dBodyID ID, dReal Cd, dReal Area) {
...
dReal ALPHA_SB::angleDiff(dReal a1, dReal a2) {
  dReal diff = fmod(a1-a2+180,360);
  diff += (diff < 0) ? 180 : -180;
  return diff;
}

void ALPHA_SB::printTorque(const char *when, const dReal *torque) {
  FILE *save_fp = ofp;
  ofp = stdout;
  printf("%s", when);
  printdR3(torque);
...
void ALPHA_SB::clamp(dReal &value, dReal abs_limit) {
  if (value > abs_limit) {
    value = abs_limit;
...
void ALPHA_SB::LogBody(dBodyID b) {
...
void ALPHA_SB::LogJoint(dJointFeedback *j) {
  printdRN(j->f1,3);
  printdRN(j->t1,3);
  printdRN(j->f2,3);
  printdRN(j->t2,3);
...
void ALPHA_SB::calculateBuoyancy() {
...
bool ALPHA_SB::Report(system_status_t *S) {
  if (started) {
    struct timespec ts;
    S->system_status = 0;
...
void ALPHA_SB::Init(int argc, char **argv) {
  int c;
  optind = OPTIND_RESET;
...
void alpha_sb_sim_init_options(int argc, char **argv) {
