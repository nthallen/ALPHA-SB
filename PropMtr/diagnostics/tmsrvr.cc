...
class my_tm_data_rcvr : public TM_data_rcvr {
  public:
    ...
    ~my_tm_data_rcvr();
    static Serverside_client *new_my_tm_data_rcvr(Authenticator *auth, SubService *ss);
    bool protocol_input();
...
Serverside_client *my_tm_data_rcvr::new_my_tm_data_rcvr(Authenticator *auth, SubService *ss) {
...
bool my_tm_data_rcvr::protocol_input() {
  ...
  msg(0, "S:%04X %04X %04X %04X %04X %04X %04X %6.2lf %8.3lf %8.3lf %8d %8d %8d",
...
bool my_tm_data_rcvr::protocol_timeout() {
  // msg(0, "%s: timeout: sending synch", iname);
  synch();
  TO.Set(1, 0);
  return false;
}
...
int main(int argc, char **argv) {
  ...
  S.add_subservice(
    new SubService(
      "DG/data/PropMtr",
      my_tm_data_rcvr::new_my_tm_data_rcvr,
      &PropMtr_obj
    )
  );
  Q.connect();
